...
(defun gnu-apl--parse-function-header (line)
...
(defun gnu-apl-save-function ()
  (goto-char (point-min))
  (let ((line (gnu-apl--trim "[ \t]" (thing-at-point 'line))))
    (unless (string= (char-to-string (aref line 0)) "∇")
      (user-error "A function definition needs to begin with ∇"))
    (let ((definition (subseq 1 line)))
      (unless ))))
